...
export function ProfessionalTemplate({ data }: { data: ResumePreviewData }) {
  const {
  ...
  return (
    <div className="mx-auto min-h-[842px] w-full max-w-[600px] bg-white p-10 text-black shadow-sm">
      <header className="border-l-4 border-blue-900 pl-4">
        <h1 className="text-2xl font-bold text-blue-900">{fullName || "Your Name"}</h1>
        {jobTitle && <p className="mt-0.5 text-sm font-semibold text-gray-700">{jobTitle}</p>}
        <p className="mt-2 text-xs text-gray-600">
          {[...contactItems, ...linkItems].join("   |   ")}
        </p>
      </header>
                ...
                <div key={experience.id ?? index}>
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="text-sm font-bold text-gray-900">
                      {experience.position} · <span className="font-semibold text-blue-900">{experience.company}</span>
                    </p>
